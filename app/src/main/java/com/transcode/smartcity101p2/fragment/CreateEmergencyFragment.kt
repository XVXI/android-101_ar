...
import com.transcode.smartcity101p2.adapter.EmergencyTypeListAdapter
...
import com.transcode.smartcity101p2.presenter.CreateEmergencyFragmentPresenter
import kotlinx.android.synthetic.main.fragment_create_emergency.*
...
import com.transcode.smartcity101p2.EmergencyActivity
...
class CreateEmergencyFragment : CoreFragment(), CreateEmergencyFragmentContract.View, EmergencyTypeListAdapter.ClickItem, ImageSelectedListAdapter.ClickDeleteItem, OnMapReadyCallback {
    override fun onClickDeleteItem(position: Int) {
        ...
        if (imageList.size == 0) {
            ...
            val p = LinearLayout.LayoutParams(resources.getDimensionPixelSize(R.dimen.dp100) * imageList.size, LinearLayout.LayoutParams.MATCH_PARENT)
            recyclervieImage.layoutParams = p
    ...
    override fun updateCreateEmergency(code: String) {
        loadingDialog.dismiss()
        submit.isEnabled = true
        clearData()
        if (code == "200" || code == "1") {
            activity?.let {
                Toast.makeText(it, getString(R.string.emergency_emergency_send_success), Toast.LENGTH_LONG).show()
                adapter.setCurrentDataType(EmergencyTypeResponse.EmergencyTypeResponseData())
                ...
                (it as EmergencyActivity).moveToTab(1)
            }
        }
    }
        ...
        map?.addOnMapClickListener {
            context?.let {
                startActivityForResult(Intent(it, SelectPinLocationActivity::class.java).apply {
                    putExtra("title", getString(R.string.emergency_emergency_event_location))
                }, 777)
            }
            false
        }
...
//                }
    ...
    override fun onclickItem(res: EmergencyTypeResponse.EmergencyTypeResponseData) {
        adapter.setCurrentDataType(res)
    }

    override fun updateTypeList(list: ArrayList<EmergencyTypeResponse.EmergencyTypeResponseData>) {
        adapter.setData(list)
        adapter.notifyDataSetChanged()
    }

    companion object {
        fun newInstance(mode: Int, title: String): CreateEmergencyFragment {
            return CreateEmergencyFragment().apply {
                val bundle = Bundle()
    ...
    lateinit var presenter: CreateEmergencyFragmentPresenter
    lateinit var adapter: EmergencyTypeListAdapter
    ...
    private fun initView() {
        ...
        presenter = CreateEmergencyFragmentPresenter(this)

        context?.let {
            adapter = EmergencyTypeListAdapter(it)
        ...
        submit.setOnClickListener {
            ...
            if (adapter.getCurrentDataType().emer_type_id == null) {
                showError(getString(R.string.emergency_emergency_input_type))
                return@setOnClickListener
            }
            if (text_details.text.isNullOrEmpty()) {
                showError(getString(R.string.emergency_emergency_input_detail))
                return@setOnClickListener
            }
            if (text_name.text.isNullOrEmpty()) {
                showError(getString(R.string.emergency_emergency_input_name))
                return@setOnClickListener
            }
            if (text_tel.text.isNullOrEmpty()) {
                showError(getString(R.string.emergency_emergency_input_tel))
                return@setOnClickListener
            ...
            if (location_text.text.toString() == "" || location_text.text.toString() == "null") {
                showError(getString(R.string.emergency_emergency_event_location))
                return@setOnClickListener
            }
            ...
            val type = adapter.getCurrentDataType().emer_type_id.toString()
            val login = Hawk.get<LoginResponse>(Const.KEY_LOGIN_DATA)
            val city_id = login.authority_info?.CityId ?: "1"
            presenter.createEmergency(text_details.text.toString(),
    ...
    fun dispatchGetPictureIntent() {
        ...
        this.startActivityForResult(photoPickerIntent, CODE_GET_IMAGE)
    }
    ...
    fun setClickLatLng(lat: Double, lng: Double) {
        ...
        click_latlng?.let {
            ...
            mapboxMap?.let {
                ...
                setMarker(ll, getString(R.string.emergency_emergency_event_location))
            }
        }
    }
