...
import com.transcode.smartcity101p2.dialog.MarketShopLocationDialog
...
class MarketOrderAdapter(var context: Context) : AppBaseAdapter() {

    var list = arrayListOf<GetAllOrderResponse.GetAllOrderResponseData>()

    private var listener: ClickConfirmItem? = null
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        view.item_layout_product.removeAllViews()
...
//                    6; // ลูกค้ายกเลิก
        ...
        view.item_confirm.visibility = View.INVISIBLE

        view.item_status.text = when (data.order_status) {
            "0" -> {
                "ลูกค้าสั่ง"
            }
            "1" -> {
            ...
            "2" -> {
                view.item_confirm.visibility = View.VISIBLE
                "กำลังส่ง"
            }
            "3" -> {
                "ส่งเสร็จแล้ว"
        ...
        view.item_confirm.setOnClickListener {
            listener?.onClickConfirmItem(data)
        }

        view.item_map.setOnClickListener {
            val geo = data.geo_location?.coordinates ?: arrayListOf()
            var latlng = if (geo.size > 1) {
                LatLng(geo[0].toDouble(), geo[1].toDouble())
            } else {
                LatLng(16.054158, 103.6501564)
            }

            showShopLocationDialog(latlng)
        }
    }
    ...
    fun setConfirmClickListener(listener: ClickConfirmItem) {
        this.listener = listener
    }

    interface ClickConfirmItem {
        fun onClickConfirmItem(item: GetAllOrderResponse.GetAllOrderResponseData)
    }

    private fun showShopLocationDialog(latlng: LatLng) {
        context?.let {
            val marketShopLocationDialog = MarketShopLocationDialog(it, latlng, "ที่อยู่ของคุณ")
