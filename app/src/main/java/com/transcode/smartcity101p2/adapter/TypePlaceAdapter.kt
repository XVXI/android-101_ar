...
import com.transcode.smartcity101p2.presenter.TypePlaceAdapterPresenter
import kotlinx.android.synthetic.main.item_sugest_place.view.*
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class TypePlaceAdapter(var context: Context) : AppBaseAdapter(), TypePlaceAdapterContract.View {
    ...
    var list = arrayListOf<PlaceListByTypeResponse.PlaceMaker>()
    private var listener: AllSearchAdapter.AllSearchAdapterClickItem? = null

    var presenter = TypePlaceAdapterPresenter(this)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.item_plan_text_remark.text = data.PlaceName

        holder.itemView.item_plan_image.load(data.ImagePath ?: "", R.mipmap.icon_travel_header)

        holder.itemView.setOnClickListener {
            listener?.onAllSearchAdapterClickItem(data.PlaceName ?: "", data.PlaceId ?: "")
        }

        var isFav = false

        for (fav in favPlaceList) {
            if (data.PlaceId == fav.place_id) {
                isFav = true
                break
            }
        }
    ...
    fun setData(list: ArrayList<PlaceListByTypeResponse.PlaceMaker>) {
        this.list = list
